...
clinicalA.cdrcat=ones(length(subjects),1);clinicalA.cdrcat(clinicalA.cdrcat==1)=NaN; clinicalA.cdrcat(clinicalA.cdr==0)=0;clinicalA.cdrcat(clinicalA.cdr==0.5)=1;
...
for i=1:length(part_icad25(1,:))
        allobservations=clean(part_icad25(:,i),3); clin=clinicalA(~isnan(allobservations),:);p=prop_signal(~isnan(allobservations));m=mean_fd(~isnan(allobservations)); a=allobservations(~isnan(allobservations)); 
        i
...
square_T = square_T_full + square_T_part'; 
figure;imagesc(square_T); set(gca,'XTick',[1:21], 'YTick',[1:21]); %clear square_* 

ids=1:210;figure(131);
for i=ids(p_perm_edu<0.01)
    j=find(i==ids(p_perm_edu<0.01))
    allobservations=part_icad25(:,i);
    clear T;T=table(clinicalA.Education, clinicalA.risk, clinicalA.cdrcat, clinicalA.Age, clinicalA.M_F,prop_signal,mean_fd, abs(clinicalA.delta_AmyloidA), clean(allobservations,3) ); 
    T.Properties.VariableNames={'edu', 'risk', 'cdrcat', 'Age', 'M_F','prop_signal', 'mean_fd', 'abs_delta_AmyloidA', 'FC'}; T=T(ix,:);
    mdl = fitlm(T,'FC~edu+risk+cdrcat+Age+M_F+prop_signal+mean_fd+abs_delta_AmyloidA+Age*M_F'); anovamdl=anova(mdl);
    mdl_p(j)=mdl.coefTest;
    subplot(3,5,j); scatter(T.edu(T.cdrcat==0), mdl.Fitted(T.cdrcat==0), 5,'filled', 'b'); hold on; scatter(T.edu(T.cdrcat==1), mdl.Fitted(T.cdrcat==1), 5,'filled', 'r');
...
ix=amyloidA.PET_fSUVR_rsf_TOT_CORTMEAN~=0 & abs(clinicalA.delta_cog_t)<365 & clinicalA.dx2num==0 & (clinicalA.apoe==34|clinicalA.apoe==44|clinicalA.apoe==33);%ix=logical(ones(675,1)); 
  ...
  clear T;T=table(cognitiveA.tmt_cor, cognitiveA.BOSTON,cognitiveA.MEMUNITS, cognitiveA.aLOGIMEM, clinicalA.Education, clinicalA.risk, clinicalA.cdrcat, clinicalA.Age, clinicalA.M_F, abs(clinicalA.delta_cog_t),   abs(clinicalA.delta_AmyloidA) ); 
  T.Properties.VariableNames={'TMT', 'BOSTON', 'MEMUNITS', 'LOGIMEM','education', 'risk', 'cdrcat', 'Age', 'M_F', 'abs_delta_cogA', 'abs_delta_AmyloidA'};T=T(ix,:);

 mdl = fitlm(T,'LOGIMEM~education+risk+cdrcat+Age+M_F+abs_delta_AmyloidA+abs_delta_cogA+abs_delta_cogA+Age*M_F')
  subplot(2,2,3);
  ...
  mdl = fitlm(T,'MEMUNITS~education+risk+cdrcat+Age+M_F+abs_delta_AmyloidA+abs_delta_cogA+Age*M_F')
  subplot(2,2,4);
  scatter(T.education(T.cdrcat==0), mdl.Fitted(T.cdrcat==0), 5,'filled', 'b'); hold on; scatter(T.education(T.cdrcat==1), mdl.Fitted(T.cdrcat==1), 5,'filled', 'r');
    m= fitlm(T.education,mdl.Fitted);
    [ypred,yci] = predict(m,T.education, 'Alpha',0.001); hold on
...
square_P_full = zeros(D); square_P_full(triu(ones(D),1)>0) = p_perm_inter(tmp); 
square_T_full = zeros(D); square_T_full(triu(ones(D),1)>0) = Tinteract(tmp); square_T_full(square_P_full>0.05)=0;
